...
export function AICoach({ trades }: AICoachProps) {
  ...
  return (
    ...
    >
      <motion.header variants={itemVars} className="flex flex-col md:flex-row md:justify-between md:items-end gap-6 border-b border-white/5 pb-4">
        <div>
          ...
          <p className="text-gray-400 text-lg">Deep-learning structural analysis of your execution patterns.</p>
        </div>
        <button 
          onClick={generate}
          disabled={loading}
        ...
        >
          ...
          <div className="relative z-10 flex items-center space-x-2 text-white font-bold tracking-wide">
      ...
      {loading ? (
        <motion.div 
          ...
          className="flex-1 premium-card flex flex-col items-center justify-center p-12 border-white/5 bg-black/40 min-h-[400px]"
        >
           ...
           <p className="text-gray-500 font-mono text-sm max-w-sm text-center">Cross-referencing {trades.length} structural executions against historical probability matrix...</p>
        </motion.div>
      ) : summary ? (
        <motion.div 
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className="flex-1 premium-card p-8 md:p-12 relative overflow-hidden border-white/10 shadow-[0_10px_40px_rgba(0,0,0,0.5)] bg-black/40"
        >
        ...
        <motion.div 
          variants={itemVars} 
          className="flex-1 premium-card p-12 text-center flex flex-col items-center justify-center border-white/5 bg-black/20 min-h-[400px]"
        >
          ...
          <h3 className="text-2xl font-display font-bold text-white mb-3 tracking-wide">Ready for Neural Insight?</h3>
          ...
          <button 
            onClick={generate}
            className="glass-button !px-8 !py-3 bg-white/5 hover:bg-white/10 border-white/10 flex items-center space-x-3 group text-indigo-300 hover:text-indigo-200"
          >
