    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400 mt-1">Organize your trading process and stay disciplined.</p>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center space-x-2 bg-gray-900/50 p-1.5 rounded-lg border border-white/10">
            <Filter size={14} className="text-gray-400 ml-1" />
            <select 
              value={groupBy} 
              onChange={e => setGroupBy(e.target.value as any)}
              className="bg-transparent text-sm text-gray-300 focus:outline-none focus:text-white"
            >
          ...
          <div className="flex items-center space-x-2 bg-gray-900/50 p-1.5 rounded-lg border border-white/10">
            <span className="text-gray-400 ml-1 text-sm">Sort:</span>
            <select 
              value={sortBy} 
        ...
        <form onSubmit={handleAddTask} className="flex flex-col gap-4">
          <div className="flex flex-col md:flex-row gap-4">
            <input 
              ...
              className="flex-1 bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-blue-500 transition-colors"
              value={newTaskTitle}
            ...
            <button 
              type="submit"
              className="bg-blue-600 hover:bg-blue-500 text-white px-6 py-3 rounded-lg font-medium flex items-center justify-center space-x-2 transition-colors whitespace-nowrap"
            >
              <Plus size={18} />
              <span>Add Task</span>
            </button>
          ...
          <div className="flex flex-wrap gap-4 items-center">
             <div className="flex items-center space-x-2 bg-white/5 border border-white/10 rounded-lg px-3 py-2">
                 ...
                 <input 
                   ...
                   className="bg-transparent border-none focus:outline-none text-sm text-white w-32 md:w-auto"
             ...
             <div className="flex items-center space-x-2 bg-white/5 border border-white/10 rounded-lg px-3 py-2">
                 ...
                 <input 
                   ...
                   className="bg-transparent border-none focus:outline-none text-sm text-white custom-date-input"
                 />
             </div>
             <div className="flex items-center space-x-2 bg-white/5 border border-white/10 rounded-lg px-3 py-2">
        ...
        {Object.entries(processedTasks).map(([groupName, groupTasks]) => {
          ...
          return (
            <div key={groupName} className="space-y-3">
              ...
              {!isCollapsed && (
                <div className="space-y-2 pl-2 border-l border-white/5 ml-3">
                  <AnimatePresence>
                    {groupTasks.length === 0 ? (
                      <motion.div 
                        initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                        className="text-gray-500 text-sm italic p-4"
                      >
                        ...
                        >
                          ...
                          <div className="flex items-center mt-4 md:mt-0 space-x-3 md:ml-4 flex-shrink-0">
                            <div className="flex flex-col sm:flex-row sm:items-center space-x-0 sm:space-x-3 gap-2">
                               ...
                               >
                                  <option value="Low" className="bg-gray-900 border-none text-white">Low</option>
                                  <option value="Medium" className="bg-gray-900 border-none text-white">Medium</option>
                                  <option value="High" className="bg-gray-900 border-none text-white">High</option>
                               </select>
