...
export function MarketSessions() {
  ...
  if (!isMarketWeekend) {
    if (openSessionsCount >= 2) {
      ...
      marketStatusColor = 'text-gray-400 dark:text-gray-500 dark:text-gray-400';
    }
  }
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto h-full flex flex-col">
      <header>
        ...
        <p className="text-gray-400 dark:text-gray-500 dark:text-gray-400">Track active global trading sessions, overlaps, and market volatility.</p>
      </header>

      {/* Top Status Panels */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="glass-panel p-6 flex flex-col items-center justify-center text-center space-y-2">
          <Clock className="w-8 h-8 text-gray-400 dark:text-gray-500 dark:text-gray-400 mb-2" />
          <div className="text-sm text-gray-400 dark:text-gray-500 dark:text-gray-400 uppercase tracking-wider font-semibold">Your Local Time</div>
          <div className="text-3xl font-mono text-gray-900 dark:text-white font-bold">
            {now.toLocaleTimeString([], { hour12: false })}
          </div>
          <div className="text-sm text-gray-400 dark:text-gray-500">{now.toLocaleDateString()}</div>
        </div>

        <div className="glass-panel p-6 flex flex-col items-center justify-center text-center space-y-2">
          <Globe2 className="w-8 h-8 text-blue-400 mb-2" />
          <div className="text-sm text-gray-400 dark:text-gray-500 dark:text-gray-400 uppercase tracking-wider font-semibold">UTC Time</div>
          <div className="text-3xl font-mono text-gray-900 dark:text-white font-bold">
            {now.toISOString().substring(11, 19)}
          </div>
          <div className="text-sm text-gray-400 dark:text-gray-500">Standard forex base time</div>
        </div>
        ...
        <div className="glass-panel p-6 flex flex-col items-center justify-center text-center space-y-2 relative overflow-hidden">
          ...
          <div className="text-sm text-gray-400 dark:text-gray-500 dark:text-gray-400 uppercase tracking-wider font-semibold">Global Market Status</div>
      ...
      <div className="glass-panel p-6">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-bold text-gray-900 dark:text-white flex items-center space-x-2">
            <Clock className="text-gray-400 dark:text-gray-500 dark:text-gray-400" />
            <span>24-Hour Session Overlap (UTC)</span>
          </h2>
          <div className="text-sm text-gray-400 dark:text-gray-500 dark:text-gray-400 flex items-center space-x-2">
            <Info size={16} />
        ...
        <div className="relative pt-8 pb-4">
          {/* Time markers */}
          <div className="flex justify-between text-xs text-gray-400 dark:text-gray-500 absolute top-0 left-0 right-0 px-[2%]">
            {[0, 3, 6, 9, 12, 15, 18, 21, 24].map(h => (
              <span key={h} className="translate-x-[-50%]">{h}:00</span>
            ))}
          </div>

          <div className="relative h-48 bg-gray-900/50 rounded-xl border border-gray-100 dark:border-white/5 mx-[2%] overflow-hidden">
            {/* Hour grid lines */}
            <div className="absolute inset-0 flex justify-between">
              {[...Array(25)].map((_, i) => (
                <div key={i} className="h-full w-px bg-white dark:bg-white/5 shadow-sm dark:shadow-none"></div>
              ))}
            </div>
            ...
            >
              <div className="bg-red-500 text-gray-900 dark:text-white text-[10px] font-bold px-1.5 py-0.5 rounded-b-md">
                NOW
              </div>
            </div>
          ...
          return (
            <div key={session.name} className={clsx(
              ...
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h3 className="text-xl font-bold text-gray-900 dark:text-white">{session.name}</h3>
                  <p className="text-xs text-gray-400 dark:text-gray-500 font-medium tracking-wider">{session.timezone}</p>
                </div>
                <div className={clsx("px-2.5 py-1 rounded-full text-xs font-bold border", 
                  isCurrentlyOpen 
                    ? `bg-emerald-500/10 text-emerald-400 border-emerald-500/20` 
                    : `bg-gray-500/10 text-gray-400 dark:text-gray-500 dark:text-gray-400 border-gray-500/20`
                )}>
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center text-sm">
                  <div className="flex flex-col">
                    <span className="text-gray-400 dark:text-gray-500 text-xs">Open (UTC)</span>
                    <span className="text-gray-600 dark:text-gray-300 font-mono">{session.openUtc.toString().padStart(2, '0')}:00</span>
                  </div>
                  <div className="w-8 h-[1px] bg-gray-700"></div>
                  <div className="flex flex-col text-right">
                    <span className="text-gray-400 dark:text-gray-500 text-xs">Close (UTC)</span>
                    <span className="text-gray-600 dark:text-gray-300 font-mono">{session.closeUtc.toString().padStart(2, '0')}:00</span>
                  </div>
                </div>
